...
class MarkhamAdmin(Resource):
    def get(self):
        from website.clothing.markham.db_helper_markham import MarkhamDbHelper

        starting_time = datetime.now()

        MarkhamDbHelper().markham_retrieve_and_clean_data()
...
class MarkhamClient(Resource):
    def get(self):
        ...
        price_decrease = [
            ...
            for i in MarkhamBestBuys.query.all()
        ]
        price_increase = [
            ProductChangeValue(i.price_change, i.title)
            for i in MarkhamWorstBuys.query.all()
        ]

        print(len(price_decrease))
        print(len(price_increase))

        cheap = MarkhamDbHelper.get_best_buys(df, price_decrease, num)
        expensive = MarkhamDbHelper.get_worst_buys(df, price_increase, num)
...
class MarkhamGetProductData(Resource):
    def get(self, title):
