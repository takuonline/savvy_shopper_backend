...
class WoolworthsClothingAdmin(Resource):
    def get(self):
        from website.clothing.woolworths_clothing.db_helper_woolworths_clothing import (
            WoolworthsClothingDbHelper,
        )

        starting_time = datetime.now()

        WoolworthsClothingDbHelper().woolworths_clothing_retrieve_and_clean_data()
...
class WoolworthsClothingClient(Resource):
    def get(self):
        ...
        price_decrease = [
            ...
            for i in WoolworthsClothingBestBuys.query.all()
        ]
        price_increase = [
            ProductChangeValue(i.price_change, i.title)
            for i in WoolworthsClothingWorstBuys.query.all()
        ]

        cheap = WoolworthsClothingDbHelper.get_best_buys(df, price_decrease, num)
        expensive = WoolworthsClothingDbHelper.get_worst_buys(df, price_increase, num)
...
class WoolworthsClothingGetProductData(Resource):
    def get(self, title):
